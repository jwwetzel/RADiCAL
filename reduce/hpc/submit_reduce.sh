...
#   qsub reduce/hpc/compile.sh                 # once, wait for it to finish
#   python3 reduce/hpc/build_manifest.py "$LOGBOOK" --capillary "LUAG" \
#           -o reduce/hpc/manifest_LUAG.csv
...
source reduce/hpc/env.sh                     # MANIFEST<-RAD_MANIFEST (now respected)
...
jid_r=$(qsub -terse "${COMMON[@]}" -N "rad_reduce_${RAD_CONFIG}" -hold_jid rad_compile \
             -t 1-"$N" "${RES[@]}" reduce/hpc/sge_reduce.sh)
echo "reduce : $jid_r   ($N tasks)"
jid_m=$(qsub -terse "${COMMON[@]}" -N "rad_merge_${RAD_CONFIG}" \
             -hold_jid "$(strip "$jid_r")" reduce/hpc/merge_reduced.sh)
echo "merge  : $jid_m"
