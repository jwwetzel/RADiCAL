...
The reducer is **bit-identical to `processRun.C`** (`radcore/validateReduce.C`
...
- `REC_DIR` in `Analysis/hpc/env.sh` points at the raw rec files
- ROOT available in batch (`setup_root` → `module load root`)

## 1. One-time compile (prebuilds `radcore/reduceRun_C.so`)
```
qsub Analysis/hpc/compile.sh        # wait for rad_compile to finish (qstat)
```
...
RAD_CONFIG=LUAG    RAD_MANIFEST=$PWD/Analysis/hpc/manifest_LUAG.csv    bash Analysis/hpc/submit_reduce.sh
...
```
`RAD_CONFIG` must match the config JSON name (`datasets/2023/configs/<RAD_CONFIG>.json`).
Each call submits a reduce array job + a dependent merge. Monitor: `qstat -u $USER`.

**Output (canonical):** `datasets/2023/reduced/<BUILD>/<E>GeV.root`
...
```
ROOT_INCLUDE_PATH=radcore:Analysis root -l -b -q 'radcore/sigmaT.C+("datasets/2023/configs/DSB1.json",150)'
ROOT_INCLUDE_PATH=radcore:Analysis root -l -b -q radcore/timingLadder.C+
```
...
```
rsync -av <argon>:<repo>/datasets/2023/reduced/  ./datasets/2023/reduced/
```
...
under `datasets/2024/raw/`, author `datasets/2024/configs/<BUILD>.json`, and run
the same flow.
