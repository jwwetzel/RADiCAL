...
#
# Outputs:
#   output/25GeV/     ntuple.root + per-energy PDFs
#   output/50GeV/
#   output/75GeV/
#   output/100GeV/
#   output/125GeV/
#   output/150GeV/    (four runs chained: RUN1258–1261, ~120k events)
#   output/Summary/   resolution_summary.pdf  timing_per_capillary.pdf
#                               summary.root
# =============================================================================
...
# pre-existing output/<E>/ntuple.root.  Used by the Argon HPC pipeline,
# where processRun is run per-run as a parallel array job and merged separately.
if [[ -n "$SKIP_PROCESS" ]]; then
    echo "Step 1: SKIPPED (SKIP_PROCESS set) — using existing output/<E>/ntuple.root"
    echo ""
else
...
root -l -b -q -e '.L Analysis/processRun.C+' > output/precompile.log 2>&1
if [[ $? -ne 0 ]]; then
    ...
    if ls Analysis/processRun_C.so &>/dev/null || \
       ls Analysis/processRun_C*.so &>/dev/null 2>/dev/null; then
        ...
        echo "  Pre-compile FAILED — see output/precompile.log"
        exit 1
    fi
fi
...
# Each writes to its own output/<label>/ directory — no conflicts.
# ---------------------------------------------------------------------------
declare -a PIDS
declare -a LABELS
...
for i in "${!PIDS[@]}"; do
    ...
    if wait "$pid"; then
        ...
        echo "  ✗ ${label} FAILED — see output/${label}.log"
        FAILED=$((FAILED+1))
    fi
done
...
echo "  Converts all PDFs to PNG and assembles output/report.html"
echo ""
...
echo ""
echo " Cross-energy summary (output/Summary/):"
...
echo "   output/report.html        — open in any browser"
echo "================================================================"
