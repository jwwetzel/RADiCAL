...
#     bash reduce/hpc/submit_all.sh
# ============================================================================
set -euo pipefail
source reduce/hpc/env.sh

[ -s "$TASKLIST" ] || { echo "ERROR: $TASKLIST missing/empty. Run: bash reduce/hpc/discover_tasklist.sh" >&2; exit 1; }
N=$(wc -l < "$TASKLIST")
...
jid_c=$(qsub -terse "${COMMON[@]}" reduce/hpc/compile.sh)
echo "compile  : $jid_c"

jid_p=$(qsub -terse "${COMMON[@]}" -hold_jid "$(strip "$jid_c")" \
             -t 1-"$N" "${PROC_L[@]}" reduce/hpc/sge_process.sh)
echo "process  : $jid_p   ($N array tasks)"

jid_m=$(qsub -terse "${COMMON[@]}" -hold_jid "$(strip "$jid_p")" reduce/hpc/sge_merge.sh)
echo "merge    : $jid_m"

jid_a=$(qsub -terse "${COMMON[@]}" -hold_jid "$(strip "$jid_m")" "${ANA_L[@]}" reduce/hpc/sge_analysis.sh)
echo "analysis : $jid_a"
