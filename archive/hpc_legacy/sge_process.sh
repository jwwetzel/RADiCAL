...
# ntuple to  output/byrun/<label>/RUN<run>/ntuple.root.
#
# Submit with the array range set to the number of tasks, e.g.
#   qsub -t 1-$(wc -l < $TASKLIST) reduce/hpc/sge_process.sh
...
echo "[task ${SGE_TASK_ID}] run=${run} E=${energy} GeV  -> output/${outlabel}"
echo "  raw: ${raw}"
