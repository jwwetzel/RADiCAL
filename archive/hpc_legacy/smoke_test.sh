...
#     cd <repo> && bash reduce/hpc/smoke_test.sh   [N_runs_per_energy]
# ============================================================================
set -uo pipefail
...
if [ -e output ] && [ ! -L output ]; then
    echo "ERROR: output is a real directory; move/remove it first." >&2; exit 1
fi

SMOKE_OUT="$RAD_WORK/Output_smoke"
SMOKE_TASKS="$RAD_WORK/tasks_smoke.txt"
mkdir -p "$SMOKE_OUT"
ln -sfn "$SMOKE_OUT" output
...
for lab in $(cut -f2 "$SMOKE_TASKS" | sort -u); do
    shopt -s nullglob; files=( output/byrun/${lab}/RUN*/ntuple.root ); shopt -u nullglob
    [ ${#files[@]} -eq 0 ] && continue
    mkdir -p "output/${lab}"
    hadd -f "output/${lab}/ntuple.root" "${files[@]}" || { echo "hadd FAILED for $lab"; exit 1; }
done
...
ln -sfn "$RAD_WORK/Output" output

echo
echo "=== SMOKE TEST exit=$rc ==="
echo "Inspect: $SMOKE_OUT/report.html , $SMOKE_OUT/Summary/"
echo "If clean -> production run:  bash reduce/hpc/submit_all.sh"
exit $rc
