...
void timingResolution()
{
    ...
    TString sumPDF = "output/Summary/timing_resolution_methods.pdf";
    ...
    // ===========================================================================
    TFile* fOut = new TFile("output/Summary/timing_summary.root", "RECREATE");
    for (int m = 0; m < kNM; ++m)  gM[m]->Write();
    for (int k = 0; k < kNC; ++k)  gCorner[k]->Write();
    fOut->Close();

    std::cout << "\n[timingResolution] Done!\n"
              << "  output/<label>/combined_timing.pdf"
                 "   (per-energy, 2 pages)\n"
              << "  output/Summary/timing_resolution_methods.pdf"
                 "  (2-page summary)\n"
              << "  output/Summary/timing_summary.root\n";
}
