...
// quantile best-bin sigma on in-memory (sum_lg, depth) pairs
static double quantBest(std::vector<std::pair<float,float>> v){
    if(v.size()<2000) return -1; std::sort(v.begin(),v.end());
    ...
    for(int b=0;b<NB;++b){ size_t lo=(size_t)b*per,hi=(b==NB-1)?v.size():lo+per; if(hi-lo<600)continue;
        ...
        if(s>22&&s<best) best=s; } return best<1e8?best:-1;   // >22: no real RADiCAL quantile (DW-UP)/2 bin is lower (DSB1 best=29.6)
}
...
void crossBuild(){
    ...
    for(int bi=0;bi<4;++bi){ const char* B=builds[bi];
        ...
        std::vector<std::vector<double>> q(nS),br(nS),Ev(nS);
        for(double E:Es){ TFile* fp=TFile::Open(radReduced(B,E)); if(!fp||fp->IsZombie())continue;
            ...
            for(int s=0;s<nS;++s){ double sq=quantBest(P[s]), sb=brightFrac(P[s],0.02);
                if(sq>0){q[s].push_back(sq);br[s].push_back(sb);Ev[s].push_back(E);} }
            fp->Close();
        }
        ...
        TGraph* g=new TGraph(q[best].size(),&Ev[best][0],&q[best][0]); g->Fit(&f,"Q");
        ...
        g->SetMarkerStyle(20+bi); g->SetMarkerColor(col[bi]); g->SetLineColor(col[bi]); g->SetMarkerSize(1.5); g->SetLineWidth(3);
