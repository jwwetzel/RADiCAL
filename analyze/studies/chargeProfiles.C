...
void chargeProfiles()
{
    ...
    for (int iRun = 0; iRun < kNRuns; ++iRun)
    {
        ...
        TString ntupleFile = TString("output/") + rc.label + "/ntuple.root";
        ...
        // ---------------------------------------------------------------------
        TString outDir = TString("output/") + rc.label;
        ...
        if (std::fabs(rc.energy_GeV - 150.) < 1.) {
            ...
            TString ratioFile = "output/Summary/_ratio_tmp.root";
    ...
    // =========================================================================
    {
        ...
        TString ampPdf = "output/Summary/hg_amplitude_vs_energy.pdf";
        ...
        TFile fAmp("output/Summary/hg_amplitude_vs_energy.root", "RECREATE");
        for (int i = 0; i < kNCap; ++i) {
            TGraph g(N);
        ...
        fAmp.Close();
        std::cout << "  -> output/Summary/hg_amplitude_vs_energy.root\n";
    }
    ...
    // =========================================================================
    {
        ...
        if (!fTmp || fTmp->IsZombie()) {
            ...
            if (anyMap) {
                ...
                TString ratioPdf = "output/Summary/hg_lg_ratio_maps.pdf";
    ...
    std::cout << "\n[chargeProfiles] Done.\n"
              << "  Per-energy PDFs : output/<label>/hg_charge_profiles.pdf\n"
              << "  Summary         : output/Summary/hg_amplitude_vs_energy.pdf\n"
              << "  Ratio maps      : output/Summary/hg_lg_ratio_maps.pdf\n";
}
