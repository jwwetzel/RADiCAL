...
static HTML report at output/report.html.
...
#lightbox { position: fixed; inset: 0; z-index: 1000; display: none;
  background: rgba(8,11,18,.90); backdrop-filter: blur(3px);
  align-items: center; justify-content: center; flex-direction: column;
  padding: 4vh 3vw; cursor: zoom-out; }
#lightbox.open { display: flex; }
#lightbox img { max-width: 94vw; max-height: 82vh; width: auto; height: auto;
  object-fit: contain; border-radius: 8px; background: #fff;
  box-shadow: 0 12px 48px rgba(0,0,0,.55); cursor: default; }
#lightbox figcaption { color: #cdd6e6; font-size: .9rem; line-height: 1.55;
  margin-top: 1rem; max-width: 980px; text-align: center; }
#lightbox .lb-close { position: fixed; top: 12px; right: 22px; color: #cdd6e6;
  font-size: 2.1rem; line-height: 1; cursor: pointer; opacity: .85; }
#lightbox .lb-close:hover { opacity: 1; }
#lightbox .lb-hint { position: fixed; bottom: 12px; left: 0; right: 0; text-align: center;
  color: #6b7689; font-size: .72rem; font-family: ui-monospace,Menlo,monospace; }
"""
...
/* click-to-zoom lightbox: click any figure image for a full-size view + caption */
(function(){
  ...
  function open(src, caption){
    img.src = src; cap.innerHTML = caption || '';
    lb.classList.add('open'); lb.setAttribute('aria-hidden','false');
  }
  // open on any figure image (but not the lightbox's own image)
  document.addEventListener('click', function(e){
    const t = e.target;
    if(t && t.tagName==='IMG' && t.closest('figure') && !t.closest('#lightbox')){
      const fig = t.closest('figure');
      const fc  = fig.querySelector('figcaption');
      open(t.getAttribute('src'), fc ? fc.innerHTML : (t.getAttribute('alt')||''));
    }
  });
  // click anywhere off the image (backdrop, caption, margins, the X) closes
  lb.addEventListener('click', close);
  // clicking the image itself keeps it open so it can be examined
...
<div id="lightbox" aria-hidden="true">
  <span class="lb-close" title="Close">&times;</span>
  <img id="lb-img" src="" alt="">
  <figcaption id="lb-cap"></figcaption>
  <div class="lb-hint">click off the image &middot; or press Esc &middot; to close</div>
</div>
...
<div class="detector-card">
  ...
  <dl>
    ...
    <dt>Fiducial cuts</dt>    <dd>Timing: r &lt; 2.5 mm (E&le;100 GeV) / 3.0 mm (E&ge;125 GeV), per-energy optimised (Layer 3) &nbsp;|&nbsp; Energy: r &lt; 2.0 mm</dd>
    ...
    return [
        ...
        Section(
            ...
            subsections=[
                ...
                Subsection(
                    ...
                    note=(
                        "The DT5742 was read with a <em>nominal</em> time axis (0.200 ns/cell), "
                        "so the only DRS4 timing artefact that can matter is the fixed per-cell "
                        "width error.  Two facts settle whether it touches the headline: the stop "
                        "cell rotates <em>uniformly</em> (left), so a data-driven correction is "
                        "well-posed; and a split-half out-of-sample test (right, trained on even "
                        "events, tested on odd) shows that correction helps only the MCP-referenced "
                        "combination, while the (DW&minus;UP)/2 corner is already immune.  The full "
                        "per-group cell-width spectra and correction-pattern fits are in the Layer-1 "
                        "appendix.  Generated by <code>drs4TimeBase.C</code>."
                    ),
                    plots=[PlotEntry(
                        sumPDF("drs4_timebase_hero.png"),
                        caption=(
                            "<strong>Left:</strong> the DRS0 G0 stop cell samples every physical "
                            "cell equally (RMS 295 vs 296 for a perfectly uniform rotation), so the "
                            "stop-cell correction is well-posed.  <strong>Right:</strong> out-of-sample "
                            "core &sigma; before/after the correction &mdash; the A&sup2; combo "
                            f"improves ({DRS4_BEF}&rarr;{DRS4_AFT} ps) because it carries the HG-to-MCP "
                            "cell gap, while the corner (DW&minus;UP)/2 estimator already cancels the "
                            "cell-width error (correction adds nothing) and the single channel is "
                            "MCP-jitter-limited."
                        ),
                        width_pct=100)],
                    finding=(
                ...
                Subsection(
                    ...
                    note=(
                        "The DRS4 noise floor (pedestal RMS) for all eight channels across the full "
                        "25&ndash;150 GeV scan, against the 5 mV threshold; saturation and spike rates "
                        "(both flat at zero) are stated.  Distilled by layer1Summary.C from "
                        "drs4Diagnostics.C."
                    ),
                    finding=(
                        "<strong>Yes &mdash; the DRS4 stays clean at every energy.</strong>  The noise "
                        "floor is flat at ~1.3 mV across all channels and all six energies, far below "
                        "the 5 mV threshold; HG saturation is &lt;0.1% even at 150 GeV (energy is read "
                        "on the unsaturated low-gain chain); the worst spike rate is ~0.2%.  No channel "
                        "degrades with beam rate or energy."
                    ),
                    plots=[PlotEntry(
                        sumPDF("layer1_drs4clean.png"),
                        caption=("DRS4 noise floor vs beam energy, all 8 capillaries &mdash; flat at "
                                 "~1.3 mV, well under the 5 mV threshold, with saturation and spikes "
                                 "negligible at every energy."),
                        width_pct=66)],
                ),
                Subsection(
                    ...
                    note=(
                        "If two channels shared an electronic path, their pulse <em>amplitudes</em> would "
                        "track each other event-by-event.  The 8&times;8 matrix below is the Pearson "
                        "correlation &rho; of every high-gain channel pair at 150&nbsp;GeV &mdash; the "
                        "single cleanest test of whether the channels are independent.  "
                        "Generated by <code>channelIntegrity.C</code>."
                    ),
                    finding=(
                        "<strong>The channels are independent &mdash; the correlations are physics, not "
                        "electronics.</strong>  Every pair is strongly correlated (&rho;&nbsp;=&nbsp;0.86&ndash;1.00) "
                        "because one EM shower lights the whole capillary bundle at once &mdash; "
                        "<em>common-mode shower sharing</em>, not cross-talk.  The tell: the least-correlated "
                        "channel is SE-D (the geometric outlier), exactly as shower geometry predicts; and "
                        "subtracting a neighbour makes the per-sample residual noise <em>worse</em>, not better, "
                        "which rules out a shared electronic path."
                    ),
                    plots=[
                        PlotEntry(
                            sumPDF("layer1_channelcorr.png"),
                            "Inter-channel amplitude correlation matrix at 150&nbsp;GeV (Pearson &rho;). "
                            "Warm/light = strongly correlated; dark = the weakly-correlated SE-D outlier.",
                            width_pct=66,
                        )
                    ],
                ),
                ...
                Subsection(
                    ...
                    note=(
                        "Behind every chapter above sits a full ten-panel quality dossier for "
                        "<em>each</em> of the six beam energies &mdash; beam hit map, MCP "
                        "amplitudes, per-channel HG/LG, time-walk, TOT &amp; LED, cut-optimisation "
                        "scans, fiducial overview, and Pb-glass containment (60 panels in all, "
                        "generated by <code>qualityPlots.C</code>).  Rather than reprint all of "
                        "them, here is the single overview page at the headline energy; the "
                        "complete set ships with the data release."
                    ),
                    finding=(
                        "<strong>Nothing.</strong>  The 150&nbsp;GeV overview is representative of "
                        "all six energies &mdash; a clean, well-centred beam spot inside the "
                        "fiducial, symmetric x/y profiles, both MCP references in range with only "
                        "~1.3% saturation, and 9.4% of events surviving the 3&nbsp;mm fiducial.  "
                        "Every energy tells the same story (live channels, clean pulses, sound "
                        "time base, linear response) with no energy-specific pathology.  The "
                        "hardware is verified end to end; on to the timing reference."
                    ),
                    plots=[
                        PlotEntry(
                            sumPDF("quality_overview_150.png"),
                            caption=(
            ...
            appendix_label=("Full Layer 1 diagnostics -- DRS4 time-base study, raw pages "
                            "(click to expand)"),
            appendix_subsections=[
                Subsection(
                    anchor="l1-timebase-raw",
                    title="DRS4 time-base verification -- raw diagnostic pages",
                    plots=[
                        PlotEntry(
                            sumPDF("drs4_timebase.pdf"),
                            caption="Full DRS4 time-base study (drs4TimeBase.C).",
                            page_captions=[
                                "Per-cell width RMS by readout group.  The DRS0 timing groups "
                                "(D0G0/D0G1) read &lt;1 ps -- nominal, no per-cell width "
                                "calibration applied; the D1 groups (LG/wire-chambers, not used "
                                "for precision timing) read ~4-5 ps.  Dashed line = 3 ps "
                                "calibration threshold.",

                                "DRS0 G0 stop-cell distribution under the asynchronous trigger.  "
                                "RMS 295 vs 296 for a perfectly uniform rotation -- every physical "
                                "cell is sampled equally, so the data-driven stop-cell correction "
                                "is well-posed.",

                                "Stop-cell correction patterns (offset vs stop cell) for the single "
                                "channel, the A&sup2; combo, and the corner estimator, with the "
                                "out-of-sample core &sigma; before/after each.  The corner shows a "
                                "fixed pattern that cancels in the difference; the combo carries the "
                                "HG-to-MCP cell gap and improves once corrected.",
                            ],
                        ),
                    ],
                    finding=(
                        "These are the raw pages behind the hero above.  The verdict is unchanged: "
                        "cell widths are nominal, the stop cell rotates uniformly, and only the "
                        "MCP-referenced combination benefits from the correction -- the corner "
                        "estimator that sets the headline already cancels the cell-width error."
                    ),
                ),
            ],
        ),
        ...
        Section(
            ...
            intro=(
                ...
                "(b) a beam position within the timing fiducial (per-energy optimised, "
                "r &lt; 2.5&ndash;3.0 mm; see the fiducial-optimisation chapter), "
            ...
            subsections=[
                    ...
                    plots=[
                        PlotEntry(
                            ...
                            caption=("Wire-chamber beam illumination at 150 GeV.  The electron beam "
                                     "is well-centred inside the timing (r&lt;3&thinsp;mm at 150 GeV) and "
                                     "energy (r&lt;2&thinsp;mm) fiducial circles (horizontal bands are the "
                                     "wire-chamber granularity).  The timing radius is optimised per "
                                     "energy &mdash; see the fiducial-optimisation chapter below."),
                            width_pct=50,
                        ),
                        PlotEntry(
                ...
                Subsection(
                    anchor="l3-fiducial-opt",
                    title="Optimising the timing fiducial &mdash; a per-energy, OOS-validated cut",
                    note=(
                        "The hero above draws the energy fiducial at r&nbsp;&lt;&nbsp;2&nbsp;mm (tight, "
                        "for shower containment).  The <em>timing</em> fiducial we set by explicit "
                        "optimisation rather than assumption.  Reusing the headline&rsquo;s exact "
                        "Method-A formula, event cuts, data-derived centroid and Gaussian-core fit, we "
                        "scan the corner &sigma;<sub>t</sub> versus fiducial radius at <em>all six "
                        "energies</em>, two complementary ways: <strong>(1) the joint optimisation</strong> "
                        "&mdash; the actual single-best-E<sub>meas</sub>-bin estimator re-selected at each "
                        "radius (literally what sets the headline; our replica reproduces the per-energy "
                        "&sigma;<sub>t</sub> to &lt;0.5&nbsp;ps), and <strong>(2) a stable selection</strong> "
                        "&mdash; a fixed top-2% / top-10% by &Sigma;A<sub>LG</sub> that freezes the "
                    ...
                    plots=[
                        PlotEntry(
                            sumPDF("fiducial_timing_scan_bestbin.png"),
                            caption=(
                                "<b>The joint view</b> &mdash; best-bin (headline) &sigma;<sub>t</sub> vs "
                                "fiducial radius, one curve per energy.  In-sample (shown), tightening "
                                "appears to help at most energies &mdash; but the run-folded OOS exposes "
                                "an <em>overfit</em> at 125&nbsp;GeV (in-sample 27.1&nbsp;ps &rarr; OOS "
                                "33.1&nbsp;ps).  The OOS therefore sets the adopted per-energy cut: "
                                "<strong>2.5&nbsp;mm for E&nbsp;&le;&nbsp;100&nbsp;GeV, 3.0&nbsp;mm for "
                                "E&nbsp;&ge;&nbsp;125&nbsp;GeV</strong>."
                            ),
                            width_pct=66,
                        ),
                        PlotEntry(
                            sumPDF("fiducial_timing_scan.png"),
                            caption=(
                                "<b>The stable view, all six energies</b> &mdash; corner &sigma;<sub>t</sub> "
                                "(fixed top-2% E<sub>meas</sub>) vs radius.  With the selection frozen the "
                                "curves are smooth and show the <em>pure</em> position effect: tightening "
                                "below ~2.5&nbsp;mm helps the lower energies (the outer ring is "
                                "lower-amplitude, higher-walk), while higher energy &rarr; lower "
                                "&sigma;<sub>t</sub> overall."
                            ),
                            width_pct=66,
                        ),
                        PlotEntry(
                    ...
                    finding=(
                        ...
                        "for 25&ndash;100&nbsp;GeV, r&nbsp;&lt;&nbsp;3.0&nbsp;mm for 125&ndash;150&nbsp;GeV "
                        "&mdash; every choice OOS-validated (optimisation bias 0.0&nbsp;ps).</strong>  "
                        "Two regimes emerge from the physics.  At the lower energies the outer-ring "
                        "position/time-walk degradation dominates, so tightening to 2.5&nbsp;mm robustly "
                ...
                Subsection(
                    ...
                    note=(
                        ...
                        "inverted-kRust palette (signal dark on white).  Generated by "
                        "transverseMaps.C (150 GeV shown; all energies in Output/Summary)."
                    ),
                    plots=[PlotEntry(
            ...
            subsections=[
                ...
                Subsection(
                    ...
                    note=(
                        ...
                        "<code>timingTailAnalysis.C</code>."
                    ),
                    plots=[
                        PlotEntry(
                            sumPDF("timing_tails.png"),
                            caption=(
                                "(DW&minus;UP)/2 with its Gaussian-core fit (top-left), and core-vs-tail "
                                "comparisons of amplitude (top-right) and worst single-channel deviation "
                                "(bottom-left).  The tail events overlay the core in both &mdash; they are "
                                "<em>not</em> low-amplitude and <em>not</em> single-channel mis-reco."
                            ),
                            width_pct=66,
                        )
                    ],
                    finding=(
                        "<strong>The core &sigma; is clean.</strong>  The distribution is a Gaussian core "
                        "plus a modest non-Gaussian shoulder &mdash; ~2.6% of events beyond 2.5&sigma; "
                        "(a pure Gaussian gives 1.2%), which raises the trimmed RMS to ~33&nbsp;ps versus "
                        "the ~31&nbsp;ps core, a ~2&nbsp;ps effect.  Those tail events have the <em>same</em> "
                        "E<sub>meas</sub> as the core (median 5128 vs 5119&nbsp;mV) and the <em>same</em> "
                        "worst single-channel deviation (225 vs 231&nbsp;ps), so the tail is <em>not</em> "
                        "time-walk and <em>not</em> a rogue channel &mdash; it is the intrinsic non-Gaussian "
                        "shape of the per-channel CFD times, mildly enhanced by the few events missing a "
                        "channel (95% vs 100% full-8).  A negligible <strong>0.13%</strong> sits beyond "
                        "250&nbsp;ps &mdash; rare catastrophic mis-reconstruction a real experiment removes "
                        "with a one-line quality cut.  So the quoted Gaussian-core &sigma; is the right, "
                        "well-behaved figure of merit."
                    ),
                ),
                Subsection(
...
def build_report() -> None:
    ...
    print("Open with:      open output/report.html")
